...
class dataform_field_latlong extends dataform_field_base {
    public $type = 'latlong';
    ...
    var $linkoutservices = array(
          ...
          "Google Earth" => "@wwwroot@/mod/dataform/field/latlong/kml.php?d=@dataid@&fieldid=@fieldid@&rid=@recordid@",
          "Geabios" => "http://www.geabios.com/html/services/maps/PublicMap.htm?lat=@lat@&lon=@long@&fov=0.3&title=Moodle%20dataform%20item",
    ...
    function dataform_field_latlong($field = 0, $df = 0) {
    ...
    function display_edit($recordid = 0) {
        ...
        $str .= '<table><tr>';
        $str .= '<td align="right"><label for="field_'.$this->field->id.'_0'. '_'. $recordid. '">' . get_string('latitude', 'dataform') . '</label></td>'.
                '<td><input type="text" name="field_'.$this->field->id.'_0'. '_'. $recordid. '" id="field_'.$this->field->id.'_0'. '_'. $recordid. '" value="'.s($lat).'" size="10" />°N</td></tr>';
        $str .= '<tr><td align="right"><label for="field_'.$this->field->id.'_1'. '_'. $recordid. '">' . get_string('longitude', 'dataform') . '</label></td>'.
                '<td><input type="text" name="field_'.$this->field->id.'_1'. '_'. $recordid. '" id="field_'.$this->field->id.'_1'. '_'. $recordid. '" value="'.s($long).'" size="10" />°E</td></tr>';
        $str .= '</table>';
        $str .= '</fieldset>';
        $str .= '</div>';
        return $str;
    ...
    function get_search_sql($value) {
        ...
        $varcharlat = sql_compare_text("c{$this->field->id}.content");
        $varcharlong= sql_compare_text("c{$this->field->id}.content1");
        return " (c{$this->field->id}.fieldid = {$this->field->id} AND $varcharlat = '$latlong[0]' AND $varcharlong = '$latlong[1]') ";
    }
