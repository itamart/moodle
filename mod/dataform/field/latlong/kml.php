...
// A lot of this initial stuff is copied from mod/dataform/view.php
...
if ($rid) {
    ...
    if (! $data = get_record('dataform', 'id', $record->dataid)) {
    ...
    if (! $field = get_record('dataform_fields', 'id', $fieldid)) {
        error('Field ID is incorrect');
    }
    if (! $field->type == 'latlong') { // Make sure we're looking at a latlong data type!
        error('Field ID is incorrect');
    }
    if (! $content = get_record('dataform_content', 'fieldid', $fieldid, 'recordid', $rid)) {
...
if (has_capability('mod/dataform:managetemplates', $context)) {
    if (!record_exists('dataform_fields','dataid',$data->id)) {      // Brand new database!
        redirect($CFG->wwwroot.'/mod/dataform/fields.php?d='.$data->id);  // Redirect to field entry
    }
}
...
if($rid) { // List one single item
    ...
    $pm->description = "&lt;a href='$CFG->wwwroot/mod/dataform/view.php?d=$d&amp;rid=$rid'&gt;Item #$rid&lt;/a&gt; in Moodle data activity";
    $pm->long = $content->content1;
    $pm->lat = $content->content;
    echo data_latlong_kml_placemark($pm);
} else {   // List all items in turn

    $contents = get_records('dataform_content', 'fieldid', $fieldid);

    echo '<Document>';

    foreach($contents as $content) {
        $pm->name = data_latlong_kml_get_item_name($content, $field);
        $pm->description = "&lt;a href='$CFG->wwwroot/mod/dataform/view.php?d=$d&amp;rid=$content->recordid'&gt;Item #$content->recordid&lt;/a&gt; in Moodle data activity";
        $pm->long = $content->content1;
        $pm->lat = $content->content;
        echo data_latlong_kml_placemark($pm);
    }
...
function data_latlong_kml_get_item_name($content, $field) {
    ...
    if($field->param2 > 0) {
        $name = htmlspecialchars(get_field('dataform_content', 'content', 'fieldid', $field->param2, 'recordid', $content->recordid));
    ...
    if($name=='') { // Done this way so that "item #" is the default that catches any problems
        $name = get_string('entry', 'dataform') . " #$content->recordid";
    }
