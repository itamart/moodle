...
if ($id) {
    if (! $cm = get_coursemodule_from_id('dataform', $id)) {
        error('Course Module ID was incorrect');
    }
    if (! $course = get_record('course', 'id', $cm->course)) {
        error('Course is misconfigured');
    }
    if (! $data = get_record('dataform', 'id', $cm->instance)) {
    ...
    if (! $cm = get_coursemodule_from_instance('dataform', $data->id, $course->id)) {
        error('Course Module ID was incorrect');
    }
} else {
...
require_capability('mod/dataform:managetemplates', $context);

if ($userid && ($userid != $USER->id) && !has_capability('mod/dataform:viewalluserpresets', $context)) {
...
switch ($action) {
    ...
    case 'confirmdelete' :
        ...
        if ($userid > 0 and ($userid == $USER->id || has_capability('mod/dataform:manageuserpresets', $context))) {
           //ok can delete
        } else {
    ...
    case 'finishimport':
        ...
        if (!get_records('dataform_records', 'dataid', $data->id)) {
...
$strfromfile       = get_string('fromfile', 'dataform');
$strchooseorupload = get_string('chooseorupload', 'dataform');
$strusestandard    = get_string('usestandard', 'dataform');
$strchoose         = get_string('choose');
$strexport         = get_string('export', 'dataform');
$strexportaszip    = get_string('exportaszip', 'dataform');
$strsaveaspreset   = get_string('saveaspreset', 'dataform');
$strsave           = get_string('save', 'dataform');
...
helpbutton('exportzip', '', 'dataform', true, true);
echo '</td><td>';
$options = new object();
...
helpbutton('savepreset', '', 'dataform', true, true);
echo '</td><td>';
$options = new object();
...
helpbutton('importfromfile', '', 'dataform', true, true);
echo '</td><td>';
...
helpbutton('usepreset', '', 'dataform', true, true);
echo '</td><td>';
...
foreach ($presets as $id => $preset) {
    ...
    $dellink = '';
    if ($preset->userid > 0 and ($preset->userid == $USER->id || has_capability('mod/dataform:manageuserpresets', $context))) {
...
function data_presets_export($course, $cm, $data) {
    ...
    $fields = get_records('dataform_fields', 'dataid', $data->id);
    ksort($fields);
    if (!empty($fields)) {
