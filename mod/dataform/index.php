...
///////////////////////////////////////////////////////////////////////////
    ...
    $strdata = get_string('modulename','dataform');
    $strdataplural  = get_string('modulenameplural','dataform');
    ...
    if (! $datas = get_all_instances_in_course("dataform", $course)) {
    ...
    $strentries = get_string('entries', 'dataform');
    $strnumnotapproved = get_string('numnotapproved', 'dataform');
    ...
    $rss = (!empty($CFG->enablerssfeeds) && !empty($CFG->dataform_enablerssfeeds));
    ...
    foreach ($datas as $data) {
        ...
        $numrecords = count_records_sql('SELECT COUNT(r.id) FROM '.$CFG->prefix.
                'dataform_records r WHERE r.dataid ='.$data->id);
        ...
        if ($rss && $data->rssarticles > 0) {
            $rsslink = rss_get_link($course->id, $USER->id, 'dataform', $data->id, 'RSS');
        }
