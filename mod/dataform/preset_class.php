...
class Data_Preset
{
    ...
    */
    ...
    { 
        ...
        if (!empty($data_id)) {  
            if (!$this->data = get_record('dataform', 'id', $data_id)) {
                print_error('wrongdataid','dataform'); 
            } else { 
    ...
    function best_name($shortname = null) {
        ...
        $string = get_string('presetname'.$shortname, 'dataform', NULL, $this->directory.'/lang/');
    ...
    */
    function get_path() {
        ...
        if ($this->user_id > 0 && ($this->user_id == $USER->id || has_capability('mod/dataform:viewalluserpresets', $context))) {
            return $CFG->dataroot.'/data/preset/'.$this->user_id.'/'.$this->shortname;
        } else if ($this->user_id == 0) {
            return $CFG->dirroot.'/mod/dataform/preset/'.$this->shortname;
        } else if ($this->user_id < 0) {
    ...
    */
    function export() {
        ...
        $fields = get_records('dataform_fields', 'dataid', $this->id());
        ...
        if (is_array($missing_files = $this->has_all_required_files())) {
            ...
            print_error('filesnotgenerated', 'dataform', null, $missing_files); 
        }
    ...
    */
    function load_from_file($directory = null) {
        ...
        if (is_array($missing_files = $this->has_all_required_files())) {
            ...
            print_error('directorynotapreset','dataform', null, $a); 
        }
        ...
        /* Now we look at the current structure (if any) to work out whether we need to clear db
           or save the data */
        $currentfields = array();
        $currentfields = get_records('dataform_fields', 'dataid', $this->id());
    ...
    */
    function get_import_html() {
        ...
        $html .= '<input type="hidden" name="d" value="'.$this->id().'" />';
        ...
        if (!empty($currentfields) && !empty($newfields)) {
            ...
            helpbutton('fieldmappings', '', 'dataform');
            $html .= '</h3><table>';
            ...
            print_error('nodefinedfields', 'dataform'); 
        }
    ...
    */
        ...
        if (!empty($newfields)) {
            ...
            foreach ($newfields as $nid => $newfield) {
                ...
                if (array_key_exists($cid, $preservedfields)) {
                    print_error('notinjectivemap', 'dataform'); 
                } else {
    ...
    function process_action($action, $params)
    {
        ...
        if (in_array("action_$action", get_class_methods(get_class($this)))) {
            ...
            print_error('undefinedprocessactionmethod', 'dataform', null, $action); 
        }
    }
...
////////////////////
    ...
    function action_confirmdelete($params)
    {
        ...
        if (!$cm = get_coursemodule_from_instance('dataform', $this->id(), $course->id)) {
            print_error('invalidrequest'); 
        }

        $context = get_context_instance(COURSE_MODULE, $cm->id);

        if ($this->user_id > 0 and ($this->user_id == $USER->id || has_capability('mod/dataform:manageuserpresets', $context))) {
           //ok can delete
        } else {
            print_error('invalidrequest'); 
        }
        ...
        $strwarning = get_string('deletewarning', 'dataform').'<br />'.
    ...
    { 
        ...
        if (!$this->clean_files()) {
            print_error('failedpresetdelete', 'dataform'); 
        }
        rmdir($this->path);

        $strdeleted = get_string('deleted', 'dataform');
    ...
    function action_finishimport($params)
    {
        ...
        $this->shortname = $this->best_name($this->name());
        $this->directory = $this->get_path();
        $this->import();

        $strimportsuccess = get_string('importsuccess', 'dataform');
        $straddentries = get_string('addentries', 'dataform');
        $strtodatabase = get_string('todatabase', 'dataform');
        if (!get_records('dataform_records', 'dataid', $this->id())) {
            notify('$strimportsuccess <a href="edit.php?d=' . $this->id() . "\">$straddentries</a> $strtodatabase", 'notifysuccess');
        } else {
    ...
    function action_export($params)
    {
        ...
        $file = $this->export();
        $html .= get_string('exportedtozip', 'dataform')."<br />";
        $permanentfile = $CFG->dataroot.'/' . $course->id . '/moddata/data/' . $this->id() . '/preset.zip';
        @unlink($permanentfile);
        /* is this created elsewhere? sometimes its not present... */
        make_upload_directory($course->id . '/moddata/data/' . $this->id());

        /* now just move the zip into this folder to allow a nice download */
        if (!rename($file, $permanentfile)) {
            print_error('movezipfailed', 'dataform'); 
        }
        
        require_once($CFG->libdir.'/filelib.php');
        $html .= '<a href="'. get_file_url($course->id .'/moddata/data/'. $this->id() .'/preset.zip') .'">'. get_string('download', 'dataform') .'</a>';
        $html .= '</div>'; 
        return $html;
    }
    ...
    function action_save1($params)
    {
        ...
        $strcontinue = get_string('continue');
        $strwarning = get_string('presetinfo', 'dataform');
        ...
        $html .= '<fieldset class="invisiblefieldset">';
        $html .= '<label for="shortname">'.$strname.'</label> <input type="text" id="shortname" name="name" value="'.$this->best_name($this->name()).'" />';
    ...
    function action_save2($params)
    {
        ...
        $strcontinue = get_string('continue');
        $stroverwrite = get_string('overwrite', 'dataform');
        $strname = get_string('shortname');

        $name = $this->best_name(optional_param('name', $this->name(), PARAM_FILE));
        $this->shortname = $name;
        $sesskey = sesskey();
    ...
    function action_save3($params)
    {
        ...
        notify(get_string('savesuccess', 'dataform'), 'notifysuccess'); 
    }
}
