<?php
    ...
    // This function is the main entry point to dataform module
    // rss feeds generation. Foreach dataform with rss enabled
    // build one XML rss structure.


    function dataform_rss_feeds() {
        global $CFG;
        ...
        // Check CFG->dataform_enablerssfeeds.
        else if (empty($CFG->dataform_enablerssfeeds)) {
            debugging("DISABLED (module configuration)");
        }
        // It's working so we start...
        else {
            // Iterate over all dataforms
            if ($dataforms = get_records('dataform')) {
                foreach ($dataforms as $data) {
                    ...
                    if ($data->rssarticles > 0) {
                        ...
                        if (!$firstfield = get_record_sql('SELECT id,name from '.$CFG->prefix.'dataform_fields WHERE dataid = '.$data->id.' ORDER by id', true)) {
                            continue;
                        }
                        ...
                        $filename = rss_file_name('dataform', $data);
                        ...
                        $header = rss_standard_header($course->shortname.': '.format_string($data->name,true),
                                                      $CFG->wwwroot."/mod/dataform/view.php?d=".$data->id,
                        ...
                        if (!empty($header) && !empty($articles) && !empty($footer)) {
                            ...
                            $status = rss_save_file('dataform', $data, $rss);
                        }
                        else {
                            $status = false;
                        }
