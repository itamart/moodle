...
if ($edit) {
    ...
    if (($forminput = data_submitted($CFG->wwwroot.'/mod/dataform/css.php')) && confirm_sesskey()) {
        $rec = new object();
        $rec->id = $df->id();
        $rec->css = $forminput->css;

        if (update_record('dataform', $rec)) {
            notify(get_string('csssaved', 'dataform'), 'notifysuccess');
        }
        
        add_to_log($df->course->id, 'dataform', 'css saved', 'css.php?id='. $df->cm->id. '&amp;d='. $df->id(), $df->id(), $df->cm->id);
    } else {
        echo '<div class="littleintro" style="text-align:center;padding:5px;">'.get_string('headercss', 'dataform').'</div>';
    }

    echo '<form id="cssform" action="css.php?d='.$df->id().'&amp;edit=1" method="post">';
    echo '<div>';
    ...
    print_textarea(false, 20, 72, 0, 0, 'css', $df->data->css);
    echo '</td></tr>';
