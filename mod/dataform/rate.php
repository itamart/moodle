<?php  // $Id$
    ...
    if (!$data = get_record('dataform', 'id', $dataid)) {
    ...
    require_capability('mod/dataform:rate', $context);
...
/// Calculate scale values
    ...
    foreach ((array)$frmdata as $recordid => $rating) {
        ...
        if (!$record = get_record('dataform_records', 'id', $recordid)) {
    ...
    /// Check rate is valid for that database scale values
        if (!array_key_exists($rating, $scale_values) && $rating != -999) {
            print_error('invalidrate', 'dataform', '', $rating);
        }
        ...
        if ($oldrating = get_record('dataform_ratings', 'userid', $USER->id, 'recordid', $record->id)) {
            if ($rating == -999) {
                delete_records('dataform_ratings', 'userid', $oldrating->userid, 'recordid', $oldrating->recordid);
                dataform_update_grades($data, $record->userid);

            } else if ($rating != $oldrating->rating) {
                $oldrating->rating = $rating;
                if (! update_record('dataform_ratings', $oldrating)) {
            ...
            if (! insert_record('dataform_ratings', $newrating)) {
    ...
    if (!empty($_SERVER['HTTP_REFERER'])) {
        redirect($_SERVER['HTTP_REFERER'], get_string('ratingssaved', 'dataform'));
    } else {
        // try to guess where to return
        if ($count == 1) {
            redirect('view.php?mode=single&amp;rid='.$record->id, get_string('ratingssaved', 'dataform'));
        } else {
            redirect('view.php?d='.$data->id, get_string('ratingssaved', 'dataform'));
        }
    }
