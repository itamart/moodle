<?php  // $Id$
    ...
    require_once('field/_comment/field_form.php');
    ...
    require_capability('mod/dataform:comment', $context);

    if ($commentid) {
        if (! $comment = get_record('dataform_comments', 'id', $commentid)) {
            error('Comment ID is misconfigured');
        }
        if ($comment->recordid != $record->id) {
            error('Comment ID is misconfigured');
        }
        if (!has_capability('mod/dataform:managecomments', $context) && $comment->userid != $USER->id) {
    ...
    $mform = new mod_dataform_comment_form();
    ...
    switch ($mode) {
        case 'add':
            ...
            if (insert_record('dataform_comments',$newcomment)) {
                redirect('view.php?rid='.$record->id.'&amp;page='.$page);
            } else {
        ...
        case 'edit':    //print edit form
            ...
            if (update_record('dataform_comments',$updatedcomment)) {
                redirect('view.php?rid='.$record->id.'&amp;page='.$page);
            } else {
                error('Error while saving comment.');
            }
        ...
        case 'delete':    //deletes single comment from db
            if ($confirm and confirm_sesskey() and $comment) {
                delete_records('dataform_comments','id',$comment->id);
                redirect('view.php?rid='.$record->id.'&amp;page='.$page, get_string('commentdeleted', 'dataform'));

            } else {    //print confirm delete form
                print_header();
                $df->print_comment($data, $comment, $page);

                notice_yesno(get_string('deletecomment','dataform'),
    ...
    print_header();
    $df->print_comments($data, $record, $page, $mform);
    print_footer();
