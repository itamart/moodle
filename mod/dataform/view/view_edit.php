...
// no submit buttons: reset to default, switch editor    
    ...
    if ($resettodefault) {   // reset view to default
        ...
        redirect($CFG->wwwroot.'/mod/dataform/view/view_edit.php?d='. $d. '&amp;vid='. $vid. '&amp;type='. $type. '&amp;default=1');
        
    } else if ($switcheditor) {    // switch editor in the view design
        // TODO is this the best way?
        redirect($CFG->wwwroot.'/mod/dataform/view/view_edit.php?d='. $d. '&amp;vid='. $vid. '&amp;type='. $type. '&amp;switcheditor=1');
    }
...
// process validated    
    ...
    if (!$vid) {    // add new view
        ...
        add_to_log($df->course->id, 'dataform', 'views update',
                   'views.php?d='. $df->id(). '&amp;vid=', $vid, $df->cm->id);
