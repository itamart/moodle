...
//This php script contains all the stuff to backup/restore data mod
    ...
    //Backup dataform files because we've selected to backup user info
...
function dataform_backup_one_mod($bf,$preferences,$data) {
    ...
    $status = backup_dataform_fields($bf,$preferences,$data->id);
    $status = backup_dataform_views($bf,$preferences,$data->id);
    ...
    if (backup_userdata_selected($preferences,'dataform',$data->id)) {
        ...
        $status = backup_dataform_records($bf,$preferences,$data->id);
        if ($status) {
            $status = backup_dataform_files_instance($bf,$preferences,$data->id);    //recursive copy
        }
    }
...
function backup_dataform_fields($bf,$preferences,$dataid){
    global $CFG;
    $status = true;

    $data_fields = get_records("dataform_fields","dataid",$dataid);
...
function backup_dataform_views($bf,$preferences,$dataid){
    global $CFG;
    $status = true;

    $data_views = get_records("dataform_views","dataid",$dataid);

        //If there is submissions
        if ($data_views) {
            //Write start tag
            $status =fwrite ($bf,start_tag("VIEWS",4,true));
            //Iterate over each submission
            foreach ($data_views as $fie_sub) {
                //Start submission
                $status =fwrite ($bf,start_tag("VIEW",5,true));
                //Print submission contents
                fwrite ($bf,full_tag("ID",6,false,$fie_sub->id));
                fwrite ($bf,full_tag("DATAID",6,false,$fie_sub->dataid));
                fwrite ($bf,full_tag("TYPE",6,false,$fie_sub->type));
                ...
                //End submission
                $status =fwrite ($bf,end_tag("VIEW",5,true));
            }
            //Write end tag
            $status =fwrite ($bf,end_tag("VIEWS",4,true));
        }
        return $status;
}

function backup_dataform_content($bf,$preferences,$recordid){
    global $CFG;
    $status = true;

    $data_contents = get_records("dataform_content","recordid",$recordid);
...
function backup_dataform_ratings($bf,$preferences,$recordid){
    global $CFG;
    $status = true;
    $data_ratings = get_records("dataform_ratings","recordid",$recordid);
...
function backup_dataform_comments($bf,$preferences,$recordid){
    global $CFG;
    $status = true;
    $data_comments = get_records("dataform_comments","recordid",$recordid);
...
function backup_dataform_records($bf,$preferences,$dataid){

    global $CFG;
    $status = true;

    $data_records = get_records("dataform_records","dataid",$dataid);
        //If there is submissions
        if ($data_records) {
            //Write start tag
            ...
            foreach ($data_records as $rec_sub) {
                ...
                backup_dataform_content($bf,$preferences,$rec_sub->id);
                backup_dataform_ratings($bf,$preferences,$rec_sub->id);
                backup_dataform_comments($bf,$preferences,$rec_sub->id);
